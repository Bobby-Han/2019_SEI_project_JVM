...
public class ClassFileReader {
    ...
    public Pair<byte[], Integer> readClassFile(String className, EntryType privilege) throws IOException, ClassNotFoundException {
        ...
        int pri=0;
        byte[] classfile=null;
        //注意如果privilege传入的是null的话，表明这个类没有被加载过，要使用默认优先级。
        if(privilege==null){
            pri=EntryType.USER_ENTRY;
        }
        else {
            pri=privilege.getValue();
        }
        try {
            classfile= bootClasspath.readClass(realClassName);
            pri=EntryType.BOOT_ENTRY;
            if(classfile==null){
                classfile=extClasspath.readClass(realClassName);
                pri=EntryType.EXT_ENTRY;
                if(classfile==null){
                    classfile= userClasspath.readClass(realClassName);
                    pri=EntryType.USER_ENTRY;
                }
            }
